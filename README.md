# 🚀 안녕하세요, 데이터의 가치를 API로 연결하는 백엔드 개발자 [이름]입니다.

> **"데이터 엔지니어링부터 백엔드 서버 구축까지, 데이터의 라이프사이클을 이해합니다."**
> 복잡한 데이터를 정제하여 효율적인 서비스 리소스로 전환하는 과정에 강점이 있습니다.
...
#### 🌐 Backend API
- **고성능 서버 구현:** `ts-fastify`를 활용하여 저지연(Low-latency) API 서버를 구축합니다.
- **데이터 기반 설계:** BI 도구(Qlik Sense) 분석 결과를 바탕으로 실제 서비스에 필요한 데이터 모델링을 수행합니다.
...
| 구분 | 주요 기술 | 핵심 내용 |
| :--- | :--- | :--- |
| **Data Integration** | ETL Tool, Vertica | - 대규모 트랜잭션 데이터의 ETL 파이프라인 자동화 구축<br>- 분석용 마트 설계를 통한 데이터 조회 효율성 증대 |
| **Service Backend** | TS, Fastify, Flask | - 데이터 파이프라인과 연동되는 고성능 백엔드 API 개발<br>- 가독성 높은 코드와 유지보수를 고려한 모듈화 설계 |
